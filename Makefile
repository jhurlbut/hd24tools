...
MORELIBS=
MOREDEPS=
TMPCCARGS= -DLIBFLTK="1.4.4"
DEBUGGING= -O3 
FLTK_HAS_FILE_CHOOSER=1
...
PLATFORM=OSX
# Note: CFLAGS and CXXFLAGS are set by configure script for current architecture
CC=g++ $(DEBUGGING) $(CXXFLAGS) -Wall -DDARWIN -DRELEASENUM=$(RELEASENUM) -DLIBFILE_SNDFILE=\"libsndfile.1.dylib\" -DLIBFILE_PORTAUDIO=\"libportaudio.2.dylib\" -DLIBFILE_JACK=\"libjack.dylib\" -DDEFAULTLIBPATH=\"/usr/lib/:/usr/local/lib/:/opt/homebrew/lib/:/lib/\"
LIBDIRS=-L/usr/local/lib -L/opt/homebrew/lib -Lsrc/lib/
INCLUDEDIRS=-I /usr/local/include -I /opt/homebrew/include -I src/frontend -I src/lib -I src/backend -I setup
CONSLIBS=-framework CoreAudio -framework AudioToolbox -framework AudioUnit -framework CoreServices `fltk-config --ldstaticflags`
RESOURCE_LIBSNDFILE_1=
